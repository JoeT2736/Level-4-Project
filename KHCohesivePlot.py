...
D = 999999999999         #domain size (square [0, D] x [0, D])
...
class HemelrijkSimulation:          #vvv change to N=None for static plots (but give a number for animation)
    def __init__(self, mode=None, N=None, size=None):#, align_scale=None):
        ...
        self.pos = np.random.uniform(D/2 - 1.25, D/2 + 1.25, size=(N, 2))
        ...
        #self.align_scale = np.where(self.size==0, align_scale, align_scale)   #currently same for small and large fish, but can be changed to have different values for small and large fish if desired
...
#centre of gravity of school
    def c_grav(self):
        # compute centroid correctly on a periodic domain [0, D)
        if self.pos.size == 0:
            return np.array([0.0, 0.0])

        # convert coordinates to angles on circle of length D
        tx = 2.0 * np.pi * self.pos[:, 0] / self.D
        ty = 2.0 * np.pi * self.pos[:, 1] / self.D

        cx = np.angle(np.mean(np.exp(1j * tx))) / (2.0 * np.pi) * self.D
        cy = np.angle(np.mean(np.exp(1j * ty))) / (2.0 * np.pi) * self.D
    ...
    def avg_c_dist(self):
        ...
        # vector from each agent to centroid, with periodic wrapping (minimum-image)
        diff = self.pos - cg
        diff = (diff + self.D / 2.0) % self.D - self.D / 2.0

        distances = np.linalg.norm(diff, axis=1)

        #only distances less than 2
        filtered_distances = [x for x in distances if x < 3]
    ...
    def near_n(self):
        ...
        # relative vectors (N, N, 2)
        rel = self.pos[:, None, :] - self.pos[None, :, :]

        # minimum-image convention for periodic domain
        rel = (rel + self.D / 2.0) % self.D - self.D / 2.0

        dist = np.linalg.norm(rel, axis=2)
        np.fill_diagonal(dist, np.inf)

        # sort distances for each focal agent
        sort_dist = np.sort(dist, axis=1)

        filtered_sort_dist1 = [x for x in sort_dist[:, 0] if x < 3]
        filtered_sort_dist2 = [x for x in sort_dist[:, 1] if x < 3]

        n1 = np.mean(sort_dist[:, 0])
        # second neighbour may not exist if N<3
        n2 = np.nan if sort_dist.shape[1] < 2 else np.mean(filtered_sort_dist2)
    ...
    def group_velocity(self):
        ...
        if speed > 0.5:  # cap speed to avoid outliers dominating the plot
            speed = 0.5
    ...
    def repel_mask(self, rel_bearing, dist):
        ...
        return mask
    '''
    def attract_mask(self, rel_bearing, dist, dist2):
    ...
    def align_mask(self, rel_bearing, dist):
        ...
        range_align = self.aligning_range[:, None] 
        #range_align = self.align_scale[:, None]
    ...
    def repel_mask(self, rel_bearing, dist):
        ...
        #mask at side of fish
        LOS2 = (np.abs(rel_bearing) <= (attract_LOS/2)) 
        mask2 = mask_distance & LOS2
    ...
    def attract_mask(self, rel_bearing, dist, dist2):
        ...
        return mask & ~self.repel_mask(rel_bearing, dist) & ~self.align_mask(rel_bearing, dist)
    
    def align_mask(self, rel_bearing, dist):
        LOS_mask = ((np.abs(rel_bearing) > (repel_LOS/2)) & (np.abs(rel_bearing) <= (attract_LOS/2)))# & (((rel_bearing) < (-repel_LOS/2)) & ((rel_bearing) >= (-attract_LOS/2)))
        range_align = self.aligning_range[:, None] 
        mask_distance = (dist <= range_align) & (dist > self.repulsion_range[:, None])
        mask = mask_distance & LOS_mask
        return mask 
    '''
    ...
    def step(self):
        ...
        ### for ellipse only model (none ellipse attraction agents ) ###
        ...
        for i in range(N):
            ...
            rotation_repel = 0 if repel_.size == 0 else np.mean(contribution)

            if (align_.size > 0) or (attract_.size > 0) or (repel_.size > 0):
        ...
        #update state
        self.pos[:, 0] = (self.pos[:, 0] + vx) 
        self.pos[:, 1] = (self.pos[:, 1] + vy) 
        ...
        return self.pos.copy(), cos, sin #, t#, self.centre_dist.copy(), self.near1.copy(), self.near2.copy(), self.Nearest_Neighbours_.copy()
    
'''
def normalise_array(arr):
    norm_arr = (arr - np.min(arr)) / (np.max(arr) - np.min(arr) + 0.00000001)
    return norm_arr
...
def run_sim(N_vals, fish_size, repeats=25):
    ...
    for mode in modes:
        ...
        for N in N_vals:
            ...
            avg_centre_norm[mode] = normalise_array(avg_centre[mode]) 
            avg_near_n_norm[mode] = normalise_array(avg_near_n[mode]) 
    ...
    return avg_centre, avg_near_n#, avg_polar_order
'''
...
def run_sim(N_vals, fish_size, repeats=20):
    modes = ['Point', 'Line', 'Ellipse']
    ...
    for mode in modes:
        ...
        for N in N_vals:
            ...
            #second filte for too large values (due to periodic boundary conditions)
            #centre_runs = [x for x in centre_runs if x < 2]
            #near_runs = [x for x in near_runs if x < 1]
    ...
    return avg_polar_order, avg_group_vel
...
markers = {'Point':'o', 'Line':'s', 'Ellipse':'d'}
...
ax = axs[0, 0]
for m in modes:
    ax.plot(N_vals, pol_order_s[m], marker=markers[m], linestyle=linestyles[m], label=m)
...
ax = axs[0, 1]
for m in modes:
    ax.plot(N_vals, pol_order_l[m], marker=markers[m], linestyle=linestyles[m])
...
ax = axs[1, 0]
for m in modes:
    ax.plot(N_vals, group_vel_s[m], marker=markers[m], linestyle=linestyles[m], label=m)
...
ax = axs[1, 1]
for m in modes:
    ax.plot(N_vals, group_vel_l[m], marker=markers[m], linestyle=linestyles[m])
ax.set_title('(d)', fontsize='14', loc='left')
ax.set_xlabel('$\\it{N}$', fontsize='14')
ax.set_ylabel(r'$\nu_g$ (m\s)', fontsize='14')
#ax.legend()
...
fig, axs = plt.subplots(2, 2, figsize=(6, 6))
ax = axs[0, 0]
labels = ['Point', 'Line', 'Ellipse']
for m in modes: 
    ax.plot(N_vals, centre_s[m], linestyle=linestyles[m], marker=markers[m])
...
ax = axs[0, 1]
for m in modes:
    ax.plot(N_vals, centre_l[m], linestyle=linestyles[m], marker=markers[m])
...
ax = axs[1, 0]
for m in modes:
    ax.plot(N_vals, near_s[m], linestyle=linestyles[m], marker=markers[m], label=m)
...
ax = axs[1, 1]
for m in modes:
    ax.plot(N_vals, near_l[m], linestyle=linestyles[m], marker=markers[m])
...
plt.tight_layout()
#plt.savefig('Centre distance and nearest neighbour, not filtered, 20x mean, D= v large, not filtered.png', dpi=400)
plt.savefig('Hemelrijk 2003 polar order parameter and group velocity,  20x mean, D=20, filtered.png', dpi=400)
#plt.show()
...
#2x2 plot as above but for different lengths of align area instead of different school sizes 
#only for ellipse model
'''
align_scale = [0.5, 2, 4]
N_vals = [3, 4, 6, 10, 25, 50, 75, 100]
fish_size = ['small', 'large']

def run_sim_align(N_vals, fish_size, align_scale, repeats=10):

    avg_centre = {scale: [] for scale in align_scale}
    avg_near_n = {scale: [] for scale in align_scale}

    #avg_centre_norm = {m: [] for m in modes}
    #avg_near_n_norm = {m: [] for m in modes}
    

    avg_polar_order = {scale: [] for scale in align_scale}
    avg_group_vel = {scale: [] for scale in align_scale}
    ...
    for scale in align_scale:
        ...
        for N in N_vals:
            ...
            for r in range(repeats):
                sim = HemelrijkSimulation(mode='ellipse', N=N, size=fish_size, align_scale=scale)
            ...
            avg_centre[scale].append(np.mean(centre_runs))
            avg_near_n[scale].append(np.mean(near_runs))
...
avg_centre_s, avg_near_s = run_sim_align(N_vals, 'small', align_scale, repeats=10)
avg_centre_l, avg_near_l = run_sim_align(N_vals, 'large', align_scale, repeats=10)



linestyles = {'0.5':'-', '2':':', '4':'-.'}
markers = {'0.5':'o', '2':'s', '4':'d'}
...
labels = ['0.5', '2', '4']
for m in modes: 
    ax.plot(N_vals, avg_centre_s[m], linestyle=linestyles[m], marker=markers[m])
ax.set_title('(a)', loc='left', fontsize='14')
ax.set_ylim(0.3, 1)
ax.set_yticks(np.arange(0.3, 1.01, 0.1))
ax.set_xticks(np.arange(0, 101, 20))
...
ax = axs[0, 1]
for m in modes:
    ax.plot(N_vals, avg_centre_l[m], linestyle=linestyles[m], marker=markers[m])
ax.set_title('(b)', loc='left', fontsize='14')
ax.set_xlabel('$\\it{N}$', fontsize='14')
ax.set_ylabel('$\\it{c}$ (m)', fontsize='14')
ax.set_ylim(0.3, 1)
...
ax = axs[1, 0]
for m in modes:
    ax.plot(N_vals, avg_near_s[m], linestyle=linestyles[m], marker=markers[m], label=m)
ax.set_title('(c)', loc='left', fontsize='14')
ax.set_xlabel('$\\it{N}$', fontsize='14')
ax.set_ylabel('$\\it{n}$ (m)', fontsize='14')
ax.set_ylim(0, 1)
...
ax = axs[1, 1]
for m in modes:
    ax.plot(N_vals, avg_near_l[m], linestyle=linestyles[m], marker=markers[m])
ax.set_title('(d)', loc='left', fontsize='14')
ax.set_xlabel('$\\it{N}$', fontsize='14')
ax.set_ylabel('$\\it{n}$ (m)', fontsize='14')
ax.set_ylim(0, 1)
...
plt.tight_layout()
plt.savefig('nearest neighbours and centre distance for different values of length of align area (large domain).png', dpi=400)
...
if __name__ == "__main__":
    ...
    #anim.save("All large, N=50, dif angle calculations.gif", dpi=400)
    #plt.show()
